...
public class TaskService {
    ...
    public TaskDto.Task updateTask(Long sessionId, Long taskId, TaskDto.TaskUpdateRequest request) {
        TaskEntity task = taskRepository
                ...
                .orElseThrow(() -> new CodeTogetherException(HttpStatus.NOT_FOUND, "Задача не найдена"));
        ...
        if (request.metadata() != null) {
            TaskDataEntity data = taskDataRepository.findByTask(task).orElseGet(() -> {
    ...
    private String serialize(Map<String, String> metadata) {
        try {
            ...
            throw new CodeTogetherException(HttpStatus.BAD_REQUEST, "Некорректные метаданные", ex.getMessage());
        }
    }
    ...
    private Map<String, String> deserialize(String json) {
        ...
        try {
            ...
            throw new CodeTogetherException(HttpStatus.INTERNAL_SERVER_ERROR, "Ошибка чтения метаданных", ex.getMessage());
        }
    }
}
