...
public class HttpOAuthProviderClient implements OAuthProviderClient {
  ...
  public OAuthUser exchangeCode(ClientRegistration registration, String code, String redirectUri) {
    ...
    Instant expiresAt =
        token.expiresAt() != null ? token.expiresAt() : Instant.now().plus(Duration.ofHours(1));
    return new OAuthUser(email, name, avatar, token.accessToken(), token.refreshToken(), expiresAt);
  }

  private TokenPayload fetchToken(
    ...
    try {
      ...
      if (!response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
        throw new CodeTogetherException(
            org.springframework.http.HttpStatus.BAD_GATEWAY, "Не удалось получить токен OAuth");
      }
      ...
      if (accessToken == null) {
        throw new CodeTogetherException(
            org.springframework.http.HttpStatus.BAD_GATEWAY, "Ответ OAuth без access_token");
      }
      ...
      throw new CodeTogetherException(
          org.springframework.http.HttpStatus.BAD_GATEWAY, "Не удалось выполнить обмен токена");
    }
  }
  ...
  private Map<String, Object> fetchUserInfo(ClientRegistration registration, String accessToken) {
    ...
    try {
      ...
      throw new CodeTogetherException(
          org.springframework.http.HttpStatus.BAD_GATEWAY, "Не удалось загрузить профиль OAuth");
    }
  }

  private String extractEmail(
      ClientRegistration registration, String accessToken, Map<String, Object> userInfo) {
  ...
  private String fallbackEmail(ClientRegistration registration, Map<String, Object> userInfo) {
    ...
    if (login == null || login.isBlank()) {
      login =
          Optional.ofNullable(asString(userInfo.get("id"))).orElse(UUID.randomUUID().toString());
    }
    if (GITHUB.equals(registration.getRegistrationId())) {
  ...
  private String extractName(
      ClientRegistration registration, Map<String, Object> userInfo, String email) {
    ...
    if (fallback == null || fallback.isBlank()) {
      fallback =
          email != null ? email.substring(0, email.indexOf('@')) : registration.getClientName();
    }
    return fallback;
  }
