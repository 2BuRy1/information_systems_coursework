...
public class PresenceService {
  ...
  public SessionPresence getPresence(Long sessionId) {
    List<CursorState> cursors =
        cursorRepository.findBySession_Id(sessionId).stream()
            .map(
                entity ->
                    new CursorState(
    ...
    List<Highlight> highlights =
        highlightRepository.findBySession_Id(sessionId).stream()
            .map(
                entity ->
                    new Highlight(
                        entity.getSession().getId(),
  ...
  public CursorState updateCursor(Long sessionId, Long userId, CursorUpdateRequest request) {
    ...
    CursorStateEntity entity =
        cursorRepository.findById(id).orElseGet(() -> buildCursorEntity(sessionId, userId));
  ...
  public Highlight updateHighlight(Long sessionId, Long userId, HighlightRequest request) {
    ...
    HighlightEntity entity =
        highlightRepository.findById(id).orElseGet(() -> buildHighlightEntity(sessionId, userId));
