...
public class SessionService {
  ...
  public SessionList listSessions(
      Long userId, Optional<String> roleFilter, int limit, Long cursor) {
    List<UserSessionEntity> memberships = userSessionRepository.findByUser_Id(userId);
    var comparator =
        Comparator.comparing((UserSessionEntity entity) -> entity.getSession().getUpdatedAt())
            .reversed();
    List<SessionSummary> summaries =
        memberships.stream()
            .filter(
                membership ->
                    roleFilter
                        .map(role -> membership.getRole().equalsIgnoreCase(role))
                        .orElse(true))
            .sorted(comparator)
            .map(
                membership ->
                    toSummary(
                        membership.getSession(), SessionRole.fromString(membership.getRole())))
            .toList();
    int startIndex = 0;
    if (cursor != null) {
  ...
  @Transactional
  public SessionMember updateRole(
      Long sessionId, Long actorId, Long targetUserId, MemberRoleRequest request) {
    ensureOwner(sessionId, actorId);
    UserSessionEntity membership =
        userSessionRepository
            .findBySession_IdAndUser_Id(sessionId, targetUserId)
            .orElseThrow(
                () -> new CodeTogetherException(HttpStatus.NOT_FOUND, "Участник не найден"));
  ...
  private SessionDetails toDetails(SessionEntity session, Long userId) {
    ...
    DocumentStats stats =
        documentService.documentStats(session.getId(), memberCount(session.getId()));
