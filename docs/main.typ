...
#let observer_req =(
  "Система должна предоставить возможность инвалидировать ссылку через 24 часа.",
...
==== Производительность
#let nfr_perf = (
  "Система должна поддерживать одновременную работу не менее 5 активных пользователей на одной доске.",
  "При таком условии информация на доске должна быть актуальной: последняя версия документа на сервере считается актуальной и доставляется всем подключенным клиентам после подтверждения операции."
)
#lines("P", nfr_perf)
...
#uc_table(
  ...
  main: [
      ...
      enum.item(2)[Система отображает invite-ссылку и кнопку "Скопировать".],
      enum.item(3)[Пользователь копирует ссылку и передает ее приглашенному участнику.],
      enum.item(4)[Приглашенный пользователь открывает ссылку.],
      enum.item(5)[Система добавляет пользователя к участникам доски.]
    )
  ],
  altA: [
    #enum(
      enum.item(1)[Пользователь нажимает "Обновить ссылку".],
      enum.item(2)[Система генерирует новую invite-ссылку и заменяет предыдущую.]
    )
  ],
...
== Диаграмма класов
#figure(
  image("./diagrams/backend.png", width: 130%)
)

= Этап 4

== Требования, которые не были выполнены и их правильная формулировка

- Редактирование названия доски не реализовано на фронте (нет UI/вызова PATCH /sessions/{id}. В коде есть только создание и просмотр): \
    Требование не было реализовано исключительно в области представления, на уровне бизнес логики патч работает
- Удаление доски на фронте отсутствует. 1/5: \
    Требование не было реализовано исключительно в области представления, на уровне бизнес логики удаление работает
- Выбор/смена языка подсветки “используемой доской” после создания не реализован на фронте (язык выбирается только при создании). 3/5: \
    Тут не совсем корректо и четко сформулировано требование. Смена должна проходить в интерфейса самой доски или главного экрана с общим списком досок?
- Логирование ошибок не выполнено 2/5: \
    Требование недостаточно точное. Логирование каких ошибок итересует стейкхолдеров, нужны уровни критичности и индикаторы.
- нету регулярного автоснапшота: \
    Требование не актуально в силу постоянного изменения документа. Версия всегда остается актуальной из-за работы на доске
- Производительность при 15 активных пользователях и актуальность данных на доске при стабильном интернет-соединении (пропускная способность не ниже 10 Мбит/с, RTT до 100 мс, потери пакетов не более 1%) не проверялись: \
    Не удалось воспроизвести требуемую нагрузку и условия подключения в рамках тестирования
