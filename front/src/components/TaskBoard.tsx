    ...
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{ flex: 1 }}
        />
        <button className="btn btn-primary" onClick={handleCreate} disabled={!text.trim()}>
          Добавить
        </button>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
        {columns.map((column) => (
          <div key={column.id} className="task-column">
            <h4 style={{ margin: '6px 0 10px' }}>{column.title}</h4>
            {tasks
                ...
                <div key={task.id} className="task-card">
                  ...
                  <div className="task-actions">
                    {columns
                      ...
                      .map((c) => (
                        <button key={c.id} className="btn btn-secondary" onClick={() => moveTask(task, c.id)}>
                          → {c.title}
                        </button>
                      ))}
                    <button className="btn btn-ghost danger" onClick={() => removeTask(task.id)}>
                      Удалить
                    </button>
                  </div>
                </div>
